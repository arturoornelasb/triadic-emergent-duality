...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item \textbf{Inverted layer emergence}: reptimeline analysis across 20 checkpoints reveals that Layer~6 (Observer) primitives activate first (step 5{,}000), while Layers~1--3 activate last (step 55{,}000). The theoretical emergence order (L1$\to$L6) is inverted during learning---abstract concepts are more statistically discriminative in GPT-2's embedding space.
    \item \textbf{Phase transition}: A sharp transition occurs at step 55{,}000 (the warmup boundary), with entropy jumping from $\sim$0 to 0.37, code churn spiking, and bit utilization tripling.
\end{enumerate}
...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item \textbf{Neural probes}: 5/8 probes returned null results. This constitutes evidence \emph{against} the framework's deeper claims, not merely a limitation: the model learns surface relational structure (analogies, some dual axes) but does \emph{not} recover the ordinal layer structure ($p$~=~0.71) or triadic dependencies ($p$~=~1.0) that are central to the theory. If the proposed algebraic chain were strongly reflected in distributional semantics, these probes should have reached significance.
