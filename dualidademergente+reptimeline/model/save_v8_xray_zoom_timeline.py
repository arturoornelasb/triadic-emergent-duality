"""Extract V8_xray_zoom timeline — ultra-high-resolution view of the explosion.

Reads checkpoints from v8_xray_zoom/ (every 50 steps, steps 30000-32000)
to capture the exact micro-sequence of the phase transition.

V8_xray paso 1 revealed the explosion occurs between step 31000 and 32000:
  - churn: 0.000 -> 0.814

This script shows whether all bits/layers activate simultaneously
or if there is a detectable micro-sequence (cascade like v9).

Results go to v8_xray_zoom_* files. Does NOT modify any prior data.
"""
import sys
...
def main():
    ...
    # Find V8_xray_zoom checkpoints
    ckpt_dir = os.path.join(SCRIPT_DIR, 'checkpoints', 'v8_xray_zoom')
    ...
    if not step_files:
        ...
        print('Run train_v8_xray_zoom.bat first.')
        sys.exit(1)
    ...
    tl_path = os.path.join(RESULTS_DIR, 'timeline_v8_xray_zoom.json')
    timeline.save_json(tl_path)
    print(f'Timeline saved: {tl_path}')

    # Save curves as CSV
    curves_path = os.path.join(RESULTS_DIR, 'v8_xray_zoom_curves.csv')
    ...
    pt_path = os.path.join(RESULTS_DIR, 'v8_xray_zoom_phase_transitions.json')
    ...
    # Generate plots
    plots_dir = os.path.join(SCRIPT_DIR, '..', 'runs', 'v8_xray_zoom', 'plots')
    ...
    print()
    print('=' * 74)
    print('  V8_XRAY_ZOOM: EXPLOSION MICRO-DYNAMICS (50-step resolution)')
    print('=' * 74)
    if 'churn_rate' in timeline.curves:
        ...
        for i in range(1, len(churn_vals)):
            ...
            if d > max_delta:
                ...
                max_delta_idx = i
        if max_delta > 0.05:
            print(f'\n  Biggest jump between step {timeline.steps[max_delta_idx-1]} '
                  f'and step {timeline.steps[max_delta_idx]}')
            print(f'  Churn: {churn_vals[max_delta_idx-1]:.4f} -> {churn_vals[max_delta_idx]:.4f} '
                  f'(delta={max_delta:.4f})')
            print(f'\n  --> Next: ultra-zoom with train_v8_xray_zoom2.bat around step '
                  f'{timeline.steps[max_delta_idx-1]} to {timeline.steps[max_delta_idx]} '
                  f'with --save-every 5')

    print('=' * 74)
    print(f'  Results:  {RESULTS_DIR}/v8_xray_zoom_*')
