"""Extract V9_xray_zoom2 timeline — maximum resolution of the explosion.

Reads checkpoints from v9_xray_zoom2/ (every 5 steps, steps 5050-5100)
to determine if the churn 0->1.0 jump is truly instantaneous or has
internal micro-structure.

Paso 2 revealed churn jumps from 0.000 to 1.000 between step 5050 and 5100.
This script captures 10 snapshots within that 50-step window.

Results go to v9_xray_zoom2_* files. Does NOT modify any prior data.
"""
import sys
...
def main():
    ...
    # Find V9_xray_zoom2 checkpoints
    ckpt_dir = os.path.join(SCRIPT_DIR, 'checkpoints', 'v9_xray_zoom2')
    ...
    if not step_files:
        ...
        print('Run train_v9_xray_zoom2.bat first.')
        sys.exit(1)
    ...
    tl_path = os.path.join(RESULTS_DIR, 'timeline_v9_xray_zoom2.json')
    timeline.save_json(tl_path)
    print(f'Timeline saved: {tl_path}')

    # Save curves as CSV
    curves_path = os.path.join(RESULTS_DIR, 'v9_xray_zoom2_curves.csv')
    ...
    pt_path = os.path.join(RESULTS_DIR, 'v9_xray_zoom2_phase_transitions.json')
    ...
    # Generate plots
    plots_dir = os.path.join(SCRIPT_DIR, '..', 'runs', 'v9_xray_zoom2', 'plots')
    ...
    print()
    print('=' * 74)
    print('  V9_XRAY_ZOOM2: EXPLOSION AT 5-STEP RESOLUTION')
    print('=' * 74)
    if 'churn_rate' in timeline.curves:
        ...
        print(f'  {"Step":>6}  {"Churn":>8}  {"Entropy":>8}  {"Util":>8}  {"dChurn":>8}')
        print(f'  {"-"*6}  {"-"*8}  {"-"*8}  {"-"*8}  {"-"*8}')
        for i, step in enumerate(timeline.steps):
            churn = churn_vals[i]
            ...
            if dc > 0.2:
                ...
                marker = ' <-- drop'
            print(f'  {step:>6}  {churn:>8.4f}  {ent:>8.4f}  {util:>8.4f}  {dc:>+8.4f}{marker}')
        ...
        for i, c in enumerate(churn_vals):
            ...
            if c > 0.95 and full_churn is None:
                full_churn = timeline.steps[i]

        if first_nonzero and full_churn:
            width = full_churn - first_nonzero
            print(f'  Churn onset:     step {first_nonzero}')
            print(f'  Churn saturated: step {full_churn}')
            print(f'  Transition width: {width} steps')
            if width <= 5:
                print(f'  -> INSTANTANEOUS: transition happens in <= 5 steps')
            elif width <= 20:
                print(f'  -> SHARP: transition happens in {width} steps')
            else:
                print(f'  -> GRADUAL: transition spreads over {width} steps')
        elif first_nonzero:
            print(f'  Churn onset at step {first_nonzero}, never saturates in window')
        else:
            print(f'  No churn detected in window')
    ...
    if len(snapshots) >= 2:
        ...
        for concept in list(base_codes.keys())[:100]:  # sample
        ...
        if first_change:
            ...
            for step in sorted(by_step.keys()):
                bits = by_step[step]
                print(f'  Step {step}: {len(bits)} bits first change — {bits[:10]}{"..." if len(bits) > 10 else ""}')

    print('=' * 74)
    print(f'  Results:  {RESULTS_DIR}/v9_xray_zoom2_*')
