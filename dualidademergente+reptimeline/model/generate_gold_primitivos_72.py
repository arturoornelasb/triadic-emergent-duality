"""
Generate gold_primitivos_72.json — V4 target file.

Each primitivo gets its own bit ON + all transitive dependency bits ON.
72 primitivos: 65 original + 7 new (bits 65-71) from circle expansion v1.1.

Usage:
    python generate_gold_primitivos_72.py
"""
...
TRANSLATIONS = {
    ...
    # --- 7 new primitivos (expansion v1.1, bits 65-71) ---
    "atracción": ("attraction", "Tendency of approach, positive valence"),
    "decaimiento": ("decay", "Progressive deterioration, dissolution of organized structure"),
    "aversión": ("aversion", "Tendency of avoidance, negative valence"),
    "cooperación": ("cooperation", "Joint action toward shared goal"),
    "pérdida": ("loss", "Absence after having had, void left by destruction"),
    "atención": ("attention", "Selective focus of consciousness, perceptual filter"),
    "intención": ("intention", "Directed purpose, will to act toward an end"),
}
...
def main():
    ...
    print('=' * 70)
    print('  GENERATING gold_primitivos_72.json (V4 targets)')
    print('=' * 70)
    print(f'  Source: {path}')
    ...
    # Jaccard of critical pairs
    print()
    print('  Critical dual pairs (Jaccard):')
    critical_pairs = [
        ('life', 'death'), ('pleasure', 'pain'), ('conscious', 'absent'),
        ('individual', 'collective'), ('receptive', 'creator_observer'),
        ('mortal_observer', 'eternal_observer'), ('attraction', 'aversion'),
    ]
    for a_key, b_key in critical_pairs:
        if a_key in gold and b_key in gold:
            bits_a = set(gold[a_key]['active_bits'])
            bits_b = set(gold[b_key]['active_bits'])
            inter = len(bits_a & bits_b)
            union = len(bits_a | bits_b)
            jaccard = inter / union if union > 0 else 0
            diff = len(bits_a ^ bits_b)
            print(f'    {a_key}/{b_key}: Jaccard={jaccard:.3f}  diff={diff} bits')

    # Save
    out_path = os.path.join(SCRIPT_DIR, 'gold_primitivos_72.json')
    ...
    # Show new primitivos
    print()
    print('  New primitivos (v1.1):')
    for key, info in gold.items():
        if info['bit'] >= 65:
            print(f'    bit {info["bit"]:2d}: {key:15s} capa={info["capa"]}  active={info["n_active"]}  deps={info["deps_es"]}')
